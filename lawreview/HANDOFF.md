...
> 마지막 업데이트: 2026-08-09 / 브랜치: `claude/korean-law-review-automation-fkmgrh`
>
...
### 조례 별표(HWP) 읽기 — 되는 것과 안 되는 것

- `HwpTextExtractor`(신규): HWP 5.0(CFB+zlib) 본문 텍스트 추출. **실파일 픽스처로 검증됨**
  (`Fixtures/daejeon_parking_annex.hwp` → 공장 200㎡/대, 근린생활 134㎡/대, 산업단지 단서까지)
- ⚠ **미해결**: 앱 실행 경로에서는 조례 값이 안 잡히고 모법으로 떨어진다.
  원인 후보 — 법제처 조례 검색이 고르는 자치법규일련번호(MST)에 따라 별표 구성이 달라짐.
  손으로 받은 MST 2098193의 별표(1.1MB 통합본)에는 부설주차장 기준이 들어 있는데,
  앱이 고른 버전의 별표에는 없었다(65KB, 주차요금표만).
  → 다음 세션에서 **앱이 실제로 고른 MST와 그 별표 목록을 찍어보고**, 필요하면
    "부설주차장" 조문이 참조하는 별표 번호(제16조 → 별표 6)로 지목해 받아야 한다.
  현재 동작은 모법 기준으로 안전하게 떨어지고 사유에 그 사실을 표기하므로 **틀린 값을
  조용히 쓰지는 않는다**(대전 공장: 조례 200이 정답인데 모법 350으로 표기됨 — 사용자 수정 필요).
...
1. **검토서 저장(DOCX) 연결** — 상단 "검토서 저장" 버튼이 아직 비어 있다.
   화면에서 고친 내용이 그대로 문서로 나가야 한다. DocxReportBuilder는 이미 있으므로
   화면 상태 → ReviewResult 매핑만 붙이면 된다
2. **AI 판정 수정 경로** — 지금 검토 결과는 읽기 전용이다. 판정이 틀렸을 때 사람이
   고칠 수 있어야 한다(사용자가 1순위로 요구한 사항)
3. **인쇄/PDF 서식 정리** — HTML이라 Ctrl+P로 되지만 인쇄용 CSS를 다듬어야 한다

### 그다음 — 용도변경 검토기 (별도 모드)

`docs/usechange-excel-analysis.md`를 먼저 읽을 것. 착수 시 사용자 확인 필요:
- 업종별 1일 오수발생량 원단위 자료(00.Reference의 계산기 파일은 2019년판 — 최신본 필요)
- 건축물대장 공공 API 연동 방식
...
```
법규검토서 ─ 신축(계획/법정) · 리모델링(변경전/변경후/법정) · 증축(동일)
용도변경   ─ (변경전/변경후/법정)
```
화면의 모드 탭은 이미 4개가 있고 열 구성(CSS)까지 바뀐다. 리모델링·증축은 데이터 흐름만 붙이면 된다.

### 이후 (사용자와 논의 후)

- 안양시 지구단위계획 수집기 (`IDistrictPlanProvider` 구현 추가 — 사용자 주 활동 지역)
- 고시문 PDF 텍스트 파싱 → 결정조서·지침 본문 추출
- 토지이용계획확인원 CAD 변환 모듈
- 법령 개정 감시 (상위법 개정 시 조례 미정비 경고)
