...
> 마지막 업데이트: 2026-08-18 / 브랜치: `claude/korean-law-review-automation-fkmgrh`
>
...
7. **최종 판정은 사람 것이다.** AI 판정·사유는 화면에서 고칠 수 있어야 하고, 고친 값이 검토서로
   나가야 한다. 단 **인용 조문은 사람도 못 고친다**(1번이 무너지므로). 3절 끝 참고.
...
└─ lawreview/
   ...
   ├─ src/LawReview.Web/    (화면 HTML/CSS/JS + 엔진 호출 API)
   ├─ src/LawReview.App/    (WebView2 셸 — 배포 대상)
   ├─ src/LawReview.Cli/    (개발·검증용 콘솔 러너 — 배포 대상 아님)
   └─ tests/LawReview.Core.Tests/
```

### 사용자 PC의 두 폴더 — 역할이 다르다 (2026-08-18 정리)

| 폴더 | 무엇 | git |
|---|---|---|
| `C:\Tools\seoul\` | **소스**. GitHub 저장소의 작업 사본(clone). 여기서 고치고 빌드·테스트·푸시한다 | ○ 그 자체가 git 작업본 |
| `C:\Tools\lawreview-dist\` | **실제로 쓰는 실행본** `LawReview.App.exe` + `00.Reference\`(원본 참조자료) | ✕ 미포함 |

- `C:\Tools\seoul`은 **원격 저장소와 이미 연결된 폴더다.** "git에서 바로 연동"과 "이 폴더에서 작업"은
  같은 말이다 — `git pull`/`push`가 그 연동이고, .NET 빌드·테스트는 로컬 파일이 있어야 돌아간다.
- `lawreview-dist`를 없애면 안 된다. `00.Reference\`의 원본(표준 서식 HWP 3종, 용도변경 가이드
  xlsm, 오수량 계산기)이 여기 있고 **분석 문서 2개가 이 경로를 근거로 인용**한다.
  회사 실무 자료라 저장소에는 넣지 않는다(공개 위험).
- **함정 — 실제로 겪은 일**: `lawreview-dist`의 exe는 손으로 복사해 왔기 때문에
  2026-07-28판(HTML 화면 전환 이전의 WinForms 버전)이 3주 넘게 그대로 남아 있었다.
  소스는 계속 고쳐졌는데 정작 쓰는 exe는 옛것이었다.
  → 이제 `build-exe.bat`이 게시 후 **`C:\Tools\lawreview-dist\`로 자동 복사**한다.
  **화면(wwwroot)을 고쳤으면 반드시 다시 빌드해야 한다** — exe에 내장되기 때문이다.

```
cd C:\Tools\seoul\lawreview
git pull
dotnet test
dotnet run --project src\LawReview.App     # 개발 중 실행
build-exe.bat                              # 배포본 갱신 → C:\Tools\lawreview-dist
```

**화면만 고칠 때는 exe를 다시 만들 필요 없다** — 개발용 호스트를 띄우고 새로고침하면 된다:

```
dotnet run --project src\LawReview.Cli -- --web 5099    # http://127.0.0.1:5099
```

이 모드는 wwwroot를 **저장소 폴더에서 직접** 읽는다(내장 사본이 아니라). 배포 exe는 내장 사본을 쓴다.
...
- **빌드된 exe는 사용자 PC `C:\Tools\lawreview-dist\LawReview.App.exe`에 배치** (git엔 미포함 — 70MB).
  `build-exe.bat`이 게시 후 자동 복사한다(2026-08-18부터). 4절 폴더 표 참고.
...
### AI 판정을 사람이 고치는 경로 (2026-08-18) — 6절 1번 완료

검토 결과가 읽기 전용이라 **틀린 판정을 고칠 방법이 없던 문제**를 해결했다.
검토서에 서명하는 것은 사람이므로 최종 판정도 사람 것이어야 한다.

- **고칠 수 있는 것**: 판정(적용/해당없음/확인필요) · 판정 사유 · 법적 기준 · 설계기준(산정식).
  **인용 조문은 고칠 수 없다** — 법제처 현행 원문만 인용한다는 원칙(2절 1번)이 무너진다.
- **원칙: 보이는 것을 고친다.** 요약표의 "설계기준(산정식)" 칸은 산정식이 없으면 판정 사유가
  대신 보이는데(실무 서식이 그렇다), 그때 그 칸을 고치면 `사유`가 고쳐진다(`JS boundField`).
  검토서 DOCX도 같은 순서(`FirstFilled(산정식, 사유)`)로 적으므로 화면과 문서가 어긋나지 않는다.
- **AI 판정 원본은 손대지 않는다.** 수정은 `state.edits`(항목Id → 고친 값)에만 쌓이고,
  행마다 ↺ 버튼으로 언제든 AI 판정으로 되돌아간다. 수정한 행은 왼쪽에 파란 띠가 붙는다.
- 검토서 저장 시 `overrides`로 함께 보내고 `JudgmentOverrides.Apply`가 **사본**에 얹는다.
  수정 내용은 자동저장·프로젝트 저장에 함께 담긴다.
- 표를 다시 그리지 않고 표시만 갱신한다(`refreshEditMarks`) — 다시 그리면 편집 중인 칸이
  사라져 탭 이동이 끊긴다.

**함께 고친 버그**: `ReviewJobs.LatestResult()`가 **마지막 작업 하나만** 돌려줬다.
기본 검토와 상세검토를 따로 돌리는 구조라, 상세를 나중에 돌리면 검토서에서 요약·인증·지구단위·
해당지번이 통째로 빠졌다. → `ReviewResults.Merge`로 항목 단위 병합(같은 항목은 나중 검토가 이기고,
행 순서는 체크리스트=표준 서식 순서로 되돌린다).
...
> 1번(AI 판정 수정 경로)은 2026-08-18에 완료 — 3절 끝 참고.
...
테스트 111건이 엔진을 지킨다. 실행: dotnet run --project lawreview/src/LawReview.App
화면(HTML)만 고칠 때는: dotnet run --project lawreview/src/LawReview.Cli -- --web 5099

[폴더 역할 — 헷갈리지 말 것]
C:\Tools\seoul       = 소스(이 저장소의 작업 사본). 고치고 빌드·테스트·푸시는 여기서.
C:\Tools\lawreview-dist = 실제로 쓰는 실행본 exe + 00.Reference(원본 참조자료, git 미포함).
                          build-exe.bat이 exe를 여기로 자동 복사한다.
...
AI 판정을 사람이 화면에서 고쳐 검토서로 내보내기(되돌리기 포함),
...
- **사람이 화면에서 고친 판정이 검토서로 나간다** (인용 조문만은 못 고침 — 법제처 원문)
