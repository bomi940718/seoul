...
> 마지막 업데이트: 2026-08-31 / 브랜치: `claude/korean-law-review-automation-fkmgrh`
>
...
| 대지 안의 공지(이격거리) | ✅ | 건축조례 별표(HWP) → 없으면 건축법 시행령 별표 2(조례로 정할 범위) |
| 장애인편의 | ✅ (모법까지) | 편의증진법 시행령 별표 1·2 **본문**을 용도별로 발췌해 인용. 조례 단계가 없다(아래) |
...
### 대지 안의 공지 + 별표 본문 인용 (2026-08-31) — 6절 1번 완료

**문제였던 것**: `setback`(대지 안의 공지) 항목이 인용하던 조례 조문은
"…기준은 별표 3과 같다" 한 줄뿐이라 **숫자가 하나도 없었다.** 값은 조례 별표(HWP)에 있다.
장애인편의 별표 1·2도 링크만 인용해 판정하는 쪽이 표를 못 보고 있었다.

- `SetbackStandardResolver` — 기초 건축조례 별표 → 광역 건축조례 별표 → **건축법 시행령 별표 2**.
  결과를 `row.CriterionText`(요약표 "법적 기준")와 조문 인용에 넣는다.
  **판정은 하지 않는다** — 각 행에 "산업단지에 건축하는 공장은 제외한다" 같은 단서가 붙어
  용도명만으로 단정할 수 없다(원칙 5). 해당될 수 있는 행을 원문 그대로 붙이고 판단은 AI·사람이 한다.
- `AnnexText` — ① 조례 별표 묶음(HWP 한 파일에 [별표 1]…[별표 N])에서 **제목으로** 한 별표만
  잘라내기, ② 법령 별표 본문을 **용도별로 발췌**(편의증진법 시행령 별표 2는 4.9만 자라 통째로는
  못 넣는다. 잘라내면 정작 필요한 행이 사라지므로 "용도로 골라서" 넣는다).
- `MolegClient.DownloadAnnexTextAsync` / `SearchOrdinanceAnnexesAsync(target=ordinbyl)`.

**조례 별표 표 모양이 지자체마다 다르다 — 이게 이 작업의 핵심 함정이었다.**
같은 "대지 안의 공지 기준"인데 실측한 것만 네 가지였고, 한 가지만 맞춰 두면 다른 지자체에서
값을 못 읽고 **조용히 모법 범위로 떨어진다**(오류가 안 나서 눈치채기 어렵다):

| 지자체 | 모양 | 함정 |
|---|---|---|
| 대전 | "가. …" 아래 "․ 준공업지역: 1.5미터 이상" (조건+거리 한 줄) | — |
| 안양 | 대상·지역구분·거리가 각각 다른 칸 → 줄이 따로 떨어짐 | 조건 문장 속 "(폭 **12미터 이상** 도로에 접한…)"을 거리로 오인 |
| 서울 | 머리표(•)를 쓰되 **면적 조건과 거리가 같은 칸** | "• 500**제곱미터** 이상"을 거리로 오인 |
| 속초 | 머리표가 하이픈("-") | — |
| 성남 | 항목명이 칸 안에서 끊김("…합계가 500제곱미터" / "이상인 공장(…)") | 끊긴 채 두면 항목명에 "공장"이 없어 **용도 매칭이 통째로 어긋남** |

그래서 형식을 분기하지 않고 **줄의 역할로만** 판단한다: 머리표를 떼고 → 거리 값이
**줄 끝에** 붙어 있으면 거리 칸(`DistanceCell`, 제곱미터는 제외) → 거리 앞에 쌓인 줄은 그 거리의
조건 → 조건 수와 거리 수가 같으면 순서대로 짝짓고, **다르면 짝을 지어내지 않는다**(없는 규칙 금지).
포괄 행("그 밖의 건축물")도 말이 제각각이라 `IsCatchAll`은 **첫머리**로만 판단한다 —
"공동주택(…스프링클러나 **그 밖에** 이와 비슷한…)"을 포괄 행으로 오인하면 엉뚱한 기준이 붙는다.

**서울특별시 건축 조례는 본문 응답에 별표가 0건**이다. 그런 조례는 자치법규 별표 검색
(`target=ordinbyl`, search=2)으로만 파일에 닿는다 — `ReviewEngine.OrdinanceAnnexLinksAsync`가
본문 별표 → (제목이 맞는 것만) → 없으면 검색 순으로 내려간다. 별표 제목이 있으면 필요한 파일만
받고, 제목이 "별표"뿐인 조례(대전)는 한 파일에 다 들어 있으므로 전부 받는다.

**장애인편의에는 조례 단계가 없다** (법제처에서 확인함 — 대전에 편의증진 관련 조례가 여럿
있지만 편의시설 설치기준을 정하는 조례는 없다). 편의증진법은 시행령 별표 1(대상시설)·
별표 2(종류·설치기준)로 끝나므로, 위계 적용 = **모법 별표 본문을 판정 근거로 넣는 것**이다.

실검증(실 API, 6개 지자체): 대전 유성구/공장·안양시/공장·서울 관악구/판매시설·
속초시/판매시설·성남시 분당구/공장·업무시설 — 앞 5개는 해당 **조례 별표 값**으로,
용도가 조례에 없는 성남/업무시설은 포괄 행으로 정확히 조회됨. 항목당 0.7~1.6초.

### AI 판정을 사람이 고치는 경로 (2026-08-18)
...
- 대지 안의 공지는 조례 별표를 읽지만, **조건이 여러 겹인 행은 조건·거리 묶음이 어긋날 수 있다**
  (안양 공동주택의 "폭 12미터 이상 도로에 접한 부분", 속초 인접경계선 판매시설 행).
  값은 다 담기고 출처도 붙지만 문장이 지저분하다. 짝을 지어내지 않는 쪽을 택한 결과다.
- 대지 안의 공지 항목의 **판정**은 여전히 AI 몫이다. 산업단지·상업지역 제외 같은 단서가 많아
  코드로 단정하지 않는다(원칙 5). 둔곡처럼 산업단지에 짓는 공장이 대표적인 예다.
- 장애인편의는 모법 별표까지만이다(조례 단계 자체가 없음 — 3절 참고). 별표 2(4.9만 자)는
  **용도로 발췌**해 넣으므로, 용도명이 별표 표기와 다르면 발췌가 비고 앞부분만 잘려 들어간다.
...
> AI 판정 수정 경로는 2026-08-18에, 대지 안의 공지·장애인편의 법 위계는 2026-08-31에 완료
> — 둘 다 3절 끝 참고.

### 1. 남은 법규에 법 위계 적용 (계속)
공개공지 확보 비율(건축조례 "공개 공지 등의 확보" 조문 — 대전 제34조 제2항이 용도별 100분의 8/5),
일조 등의 확보를 위한 높이 제한(조례 위임 부분). 패턴은 확립되어 있다(3절 표) —
`Municipality.OrdinanceHierarchy` + 조문/별표 파싱 + 못 찾으면 다음 단계.
별표에 값이 있으면 `SetbackStandardResolver`처럼 `ReviewEngine`의 항목별 보강 지점에 붙인다.
...
테스트 136건이 엔진을 지킨다. 실행: dotnet run --project lawreview/src/LawReview.App
